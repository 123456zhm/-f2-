...
class DouyinDownloader(BaseDownloader):
    ...
    async def download_video(self, video_data: Dict[str, Any]) -> Dict[str, Any]:
        aweme_id = video_data.get("aweme_id", "")
        title = video_data.get("title", "")
        desc = video_data.get("desc", "")
        user_id = video_data.get("user_id", "")
        nickname = video_data.get("nickname", "")
        create_time = video_data.get("publish_time", "")
        ...
        try:
            ...
            if self.download_cover and cover_url:
                cover_path = user_path / f"{file_name}_cover.jpg"
                await self.download_file(cover_url, cover_path)
                result["files"].append(str(cover_path))
                logger.info(f"下载封面: {cover_path}")
            ...
            if self.download_desc and desc:
                ...
                with open(desc_path, "w", encoding="utf-8") as f:
                    ...
                    f.write(f"链接: https://www.douyin.com/video/{aweme_id}\n")
            ...
            result["error"] = str(e)
            logger.error(f"下载视频 {aweme_id} 失败: {e}")
