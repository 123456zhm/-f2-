...
from f2.apps.weibo.utils import ClientConfManager, StatusIdFetcher
...
@click.command(name="weibo", help=_("微博内容解析下载"))
@click.option(
    "--config",
    "-c",
...
@click.option(
    ...
    default="one",
    help=_("下载模式: one(单条微博), post(用户微博), search(搜索)"),
)
@click.option(
    "--cookie",
    "-k",
...
@click.pass_context
def weibo(ctx: click.Context, **kwargs):
    main_conf = ConfigManager(kwargs.get("config")).get_config("weibo")
    custom_conf = kwargs
...
def get_help(app_name: str) -> None:
    from f2.apps.weibo.help import get_help as get_weibo_help
    get_weibo_help()

from f2.apps.weibo.help import get_help